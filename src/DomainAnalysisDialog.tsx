...
import { ArrowRight, ArrowUpRight, Check, ChevronRight, Clock3, Crosshair, Globe2, LoaderCircle, Network, Search, ShieldCheck, Terminal, X } from 'lucide-react'
import type { DomainConnection, DomainReport, DomainSourceId } from '../server/domainTypes'
import { domainSourceUrl, fetchDomainReport } from './domainAnalysis'
import DomainAnalysisChat from './DomainAnalysisChat'
...
export default function DomainAnalysisDialog({ model, reasoningEnabled, onClose }: { model: string; reasoningEnabled?: boolean; onClose(): void }) {
  ...
  async function analyze(event: FormEvent) {
    ...
    setLoading(true); setError(''); setReport(null); setView('요약')
  ...
  const cancel = () => { generation.current++; scanRef.current?.abort(); setLoading(false); setError('분석을 취소했습니다.') }
  ...
  return createPortal(<dialog ref={dialogRef} className="domain-dialog" aria-labelledby={titleId}
    ...
    <header className="domain-header"><div className="domain-heading"><span className="domain-heading-icon"><Terminal size={24} aria-hidden="true" /></span><div><span className="domain-eyebrow">MIRA / WEB INTELLIGENCE</span><h2 id={titleId}>웹 해킹<span className="domain-title-cursor" aria-hidden="true">_</span></h2></div></div><div className="domain-header-actions"><span className="domain-passive-badge"><span aria-hidden="true" />공개 정보 분석</span><button type="button" className="domain-close" onClick={onClose} aria-label="웹 해킹 닫기"><X size={20} aria-hidden="true" /></button></div></header>
    <div className="domain-scroll">
      <form className="domain-form" onSubmit={(event) => void analyze(event)}>
        <div className="domain-target-heading"><label htmlFor={inputId}><Crosshair size={13} aria-hidden="true" />TARGET / 대상 도메인</label><span>{loading ? '조회 중' : report ? '조회 완료' : '입력 대기'}</span></div>
        <div className="domain-input-row"><div className="domain-input-wrap"><ChevronRight size={20} aria-hidden="true" /><input ref={inputRef} id={inputId} value={input} onChange={(event) => setInput(event.target.value)} placeholder="example.com" type="text" inputMode="url" autoComplete="off" autoCapitalize="none" spellCheck={false} maxLength={2048} required aria-describedby={`${inputId}-hint`} /></div>
      ...
      {loading && <div className="domain-loading" role="status"><div className="domain-scan-track" aria-hidden="true" /><LoaderCircle className="domain-spinner" size={26} aria-hidden="true" /><span className="domain-eyebrow">COLLECTING PUBLIC SIGNALS</span><h3>공개 기록을 모으고 있습니다</h3><p>DNS · 등록정보 · 인증서 · 과거 웹 기록<br />일부 출처가 응답하지 않아도 확인된 결과를 보여줍니다.</p></div>}
      {!report && !loading && <div className="domain-intro"><div><span className="domain-module-number">01 / TRACE</span><Network size={22} aria-hidden="true" /><h3>연결의 흔적</h3><p>인증서와 공개 웹 기록에 등장한 도메인을 연결합니다.</p></div><div><span className="domain-module-number">02 / TIMELINE</span><Clock3 size={22} aria-hidden="true" /><h3>시간 속 기록</h3><p>등록·인증서·보관된 페이지를 관측 시점별로 살펴봅니다.</p></div><div><span className="domain-module-number">03 / INTELLIGENCE</span><ShieldCheck size={22} aria-hidden="true" /><h3>근거 있는 대화</h3><p>AI 해설을 읽고, 궁금한 단서에 대해 바로 질문하세요.</p></div></div>}
      {report && <>
        ...
        <div className="domain-workspace"><div className="domain-results-column">
        ...
        </div><DomainAnalysisChat key={`${report.domain}:${report.queriedAt}`} report={report} model={model} reasoningEnabled={reasoningEnabled} /></div>
      </>}
    </div>
