...
## 도로명 검색과 영상 팝업 (2026-09-06)

- `올림픽대로 CCTV 열어줘`, `경부고속도로 CCTV 보여줘`처럼 이름을 지정하면 `POST /api/cctv/search`에 `{ query, limit: 20 }`만 전송한다. 위치 권한을 요청하지 않으며, GPS나 가상의 거리 값을 만들지 않는다.
- 기존 ITS 전체 메타데이터 캐시에서 도로명 또는 CCTV 이름이 일치하는 항목을 검색한다. 검색을 먼저 수행한 뒤 최대 20곳을 반환하므로 주변 20곳 또는 반경 2km에 포함되지 않은 도로도 검색할 수 있다. `경부고속도로`는 ITS의 `경부선` 표기도 찾는다. 실제 매칭된 총개수를 표시하고, 결과가 많으면 구체적인 CCTV 이름으로 좁히도록 안내한다.
- 이름을 지정한 결과는 해당 CCTV 목록만 보여준다. 일치하는 항목이 없으면 빈 결과를 안내하며 다른 CCTV로 대체하지 않는다. 이름을 지정하지 않은 `내 주변 CCTV 보여줘`는 기존 위치·2km 지도 흐름을 사용한다.
- 지도 마커 또는 목록에서 CCTV를 선택하면 `CCTV 접근` 버튼이 표시된다. 클릭하면 최상위 native modal dialog의 glass 팝업에서 해당 영상이 음소거 자동 재생된다. native HLS와 hls.js fallback을 재사용한다.
- 닫기 버튼, Esc, 팝업 바깥 클릭으로 닫을 수 있다. 배경은 modal 동안 조작되지 않으며, 닫으면 버튼 포커스와 스크롤 상태를 복원한다. 닫기·선택 변경·컴포넌트 해제 시 기존 player와 HLS 인스턴스가 정리된다. GPS·CCTV 메타데이터·검색 결과는 대화 저장 이력에서 제외한다.
- 실제 Node API에서 `올림픽대로` 결과 1곳, `경부고속도로` 결과 388곳 중 20곳을 확인했다(제공 시점에 따라 변동). Pages 전체 route bundle을 workerd에서 실행하여 실제 검색 성공·빈 결과·캐시 재사용을 확인했다. 실제 경부동탄터널 영상은 브라우저에서 `readyState: 4`, 재생 중, 음소거 상태로 확인했고, 1280px·390px·320px 팝업 및 Esc/포커스 복귀를 확인했다.
